...
using namespace boost::python;
	...
	Sketch_STRIPS_Problem::create_negated_fluents_ext() {
		...
		for ( auto fl_idx : m_negated_conditions ) {
			...
			const auto& dlplan_atom = instance()->get_instance_info().get_atom(fl_idx);
			std::vector<std::string> object_names;
			for (const auto& object : dlplan_atom.get_objects()) {
				object_names.push_back(object.get_name());
			}
			unsigned neg_fl_idx = instance()->add_fluent( negated_signature, dlplan_atom.get_predicate().get_name(), std::move(object_names), true );
