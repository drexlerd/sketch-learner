...
State*	Fwd_Search_Problem::next( const State& s, Action_Idx a, int index ) const {
	const Action& act = *(task().actions().at(a));
	State* succ = s.progress_through( act, NULL, NULL, index );
	succ->update_hash();
	return succ;
}

State*	Fwd_Search_Problem::next( const State& s, Action_Idx a, Fluent_Vec* added, Fluent_Vec* deleted, int index ) const {
	const Action& act = *(task().actions().at(a));
	State* succ = s.progress_through( act, added, deleted, index );
	succ->update_hash();
	return succ;
}
