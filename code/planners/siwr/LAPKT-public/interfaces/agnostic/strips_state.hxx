...
public:
	...
	State( const STRIPS_Problem& p, int index );  // Extended by index
	~State();
	...
	State*	progress_through( const Action& a, Fluent_Vec* added = NULL, Fluent_Vec* deleted = NULL, int index=-1 ) const;

	State*	progress_through_df( const Action& a, int index=-1 ) const;

	State*	regress_through( const Action& a, int index=-1 ) const;
