...
public:
	...
	void	start(State*s = NULL) {
		...
		// we only need novelty table if bound > 0
		if (this->bound() > 0) {
			m_novelty->init();
...
protected:
	 ...
	 * IW(0), i.e., 1-step BFS.
	 */
	virtual Search_Node* process_iw_0( Search_Node *head ) {
		std::vector< aptk::Action_Idx > app_set;
		this->problem().applicable_set_v2( *(head->state()), app_set );
		for (unsigned i = 0; i < app_set.size(); ++i ) {
			int a = app_set[i];
			State *succ = this->problem().next( *(head->state()), a, this->m_gen_count );
			Search_Node* n = new Search_Node( succ , a, head, this->problem().task().actions()[ a ]->cost() );
			if ( this->is_closed( n ) ) {  // check if expanded
				delete n;
				continue;
			}
			if( this->previously_hashed(n) ) {  // check if generated
				delete n;
			} else {
				this->inc_gen();
				if( this->is_goal( n ) ) {
	...
	virtual Search_Node* do_search_iw_0() {
		// Note: no dequeueing, just a 1-step expansion.
		Search_Node* goal = process_iw_0( this->m_root );
        this->inc_exp();
		return goal;
	}
	...
	virtual Search_Node*   	process(  Search_Node *head ) {
		...
		for (unsigned i = 0; i < app_set.size(); ++i ) {
            ...
            //Search_Node* existing_node = this->previously_hashed(n);
			if( this->previously_hashed(n) ) {
				...
				// std::cout << n->state()->index() << std::endl;
			    // n->state()->print( std::cout );
