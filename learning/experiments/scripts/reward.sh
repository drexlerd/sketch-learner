#!/bin/bash

DOMAIN="${PWD}/../../benchmarks/reward/domain.pddl"
TASK_DIR="${PWD}/../../benchmarks/reward/instances"
WIDTH=$1
CONCEPT_COMPLEXITY=9
ROLE_COMPLEXITY=9
BOOLEAN_COMPLEXITY=9
COUNT_NUMERICAL_COMPLEXITY=9
DISTANCE_NUMERICAL_COMPLEXITY=9
WORKSPACE="${PWD}/workspace/reward_${WIDTH}_${CONCEPT_COMPLEXITY}_${ROLE_COMPLEXITY}_${BOOLEAN_COMPLEXITY}_${COUNT_NUMERICAL_COMPLEXITY}_${DISTANCE_NUMERICAL_COMPLEXITY}"

./runner.sh $DOMAIN $TASK_DIR $WORKSPACE $WIDTH $CONCEPT_COMPLEXITY $ROLE_COMPLEXITY $BOOLEAN_COMPLEXITY $COUNT_NUMERICAL_COMPLEXITY $DISTANCE_NUMERICAL_COMPLEXITY
