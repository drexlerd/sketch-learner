...
rm -rf ${WORKSPACE}
mkdir -p ${WORKSPACE}
./../../learner/main.py --domain ${DOMAIN} --task_dir ${TASK_DIR} --workspace ${WORKSPACE} -w ${WIDTH} -cc ${CONCEPT_COMPLEXITY} -rc ${ROLE_COMPLEXITY} -bc ${BOOLEAN_COMPLEXITY} -ncc ${COUNT_NUMERICAL_COMPLEXITY} -ndc ${DISTANCE_NUMERICAL_COMPLEXITY} --exp_id ${DOMAIN_NAME}:${EXPERIMENT_NAME} 2> ${RUN_ERR} 1> ${RUN_LOG}
exit_code=$?
