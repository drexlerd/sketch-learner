...
from ..iteration_data.equivalence_data import EquivalenceData, TupleGraphExt
...
class ASPFactFactory:
    def make_asp_facts(self, instance_datas: List[InstanceData], domain_feature_data: DomainFeatureData, instance_feature_datas: List[InstanceFeatureData], equivalence_data: EquivalenceData):
        ...
        self._add_domain_facts(builder, domain_feature_data, equivalence_data)
        for i in range(len(instance_datas)):
            self._add_instance_facts(i, builder, instance_datas[i], equivalence_data)
        builder.log_stats()
        return str(builder)

    def _add_domain_facts(self, builder: ASPInstanceBuilder, domain_feature_data: DomainFeatureData, equivalence_data: EquivalenceData):
        f_idx = 0
        for boolean in domain_feature_data.boolean_features:
            builder.add_boolean_feature(f_idx, boolean.compute_complexity())
            f_idx += 1
        for numerical in domain_feature_data.numerical_features:
        ...
        for r_idx, rule in enumerate(equivalence_data.rules):
            ...
            for condition in rule.get_conditions():
                ...
                if try_parse(r"\(:c_n_gt (\d+)\)", condition_str, r_idx, len(domain_feature_data.boolean_features), builder.add_c_gt): continue
                if try_parse(r"\(:c_n_eq (\d+)\)", condition_str, r_idx, len(domain_feature_data.boolean_features), builder.add_c_eq): continue
            for effect in rule.get_effects():
                ...
                if try_parse(r"\(:e_n_inc (\d+)\)", effect_str, r_idx, len(domain_feature_data.boolean_features), builder.add_e_inc): continue
                if try_parse(r"\(:e_n_dec (\d+)\)", effect_str, r_idx, len(domain_feature_data.boolean_features), builder.add_e_dec): continue
                if try_parse(r"\(:e_n_bot (\d+)\)", effect_str, r_idx, len(domain_feature_data.boolean_features), builder.add_e_bot): continue

    def _add_instance_facts(self, instance_idx: int, builder: ASPInstanceBuilder, instance_data: InstanceData, equivalence_data: EquivalenceData):
        self._add_tuple_graph_facts(instance_idx, builder, equivalence_data.instance_data_exts[instance_idx].tuple_graph_ext_by_state_index)
    ...
    def _add_tuple_graph_facts(self, instance_idx: int, builder: ASPInstanceBuilder, tuple_graphs_by_state_index: List[TupleGraphExt]):
        for root_idx in range(len(tuple_graphs_by_state_index)):
            ...
            for r_idx, s_idxs in tuple_graph.r_idx_to_s_idxs.items():
                for s_idx in s_idxs:
                    builder.add_equivalence_contains(instance_idx, r_idx, tuple_graph.root_idx, s_idx)
