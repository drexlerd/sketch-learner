...
from .iteration_data.equivalence_data import EquivalenceDataFactory, InstanceEquivalenceData
...
def run(config, data, rng):
    ...
    while not timer.is_expired():
        ...
        rules, instance_equivalence_datas = EquivalenceDataFactory().make_equivalence_class_data(selected_instance_datas, domain_feature_data, instance_feature_datas)

        # 1.3. Generate asp facts
        #facts = ASPFactFactory().make_asp_facts(selected_instance_datas, domain_feature_data, instance_feature_datas, instance_equivalence_datas)
