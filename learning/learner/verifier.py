...
def verify_sketch_for_problem_class(
    ...
    for instance_data in instance_datas:
        for numerical in [n.get_element() for n in sketch.dlplan_policy.get_numericals()]:
            ...
            for state_index, representative_index in instance_data.ss_state_idx_to_gfa_state_idx.items():
                ...
                if len(representative_feature_valuations[representative_index]) > 1:
                    print(str(numerical))
                    for state_index2, representative_index2 in instance_data.ss_state_idx_to_gfa_state_idx.items():
