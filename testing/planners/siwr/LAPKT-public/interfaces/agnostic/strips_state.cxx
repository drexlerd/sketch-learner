...
State::State( const STRIPS_Problem& problem, int index )
    : m_fluent_set( problem.num_fluents() ), m_problem( problem ), m_index(index)
{
}
...
State* State::progress_through_df( const Action& a, int index ) const
{
	assert( a.can_be_applied_on(*this) );

	State* succ = new State( problem(), index );
...
State* State::progress_through( const Action& a, Fluent_Vec* added, Fluent_Vec* deleted, int index ) const
{
...
State* State::regress_through( const Action& a, int index ) const
{
