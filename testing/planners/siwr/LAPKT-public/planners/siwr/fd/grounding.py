...
def initialize_sketch_strips_problem(domain_file, problem_file, sketch_file, output_task):
    ...
    # Initialize dynamic negative atoms
    # TODO(dominik): We currently create a negated version for all atoms.
    # This slows down the search but can decrease the width of the problem.
    ...
    print("Adding negated atoms...")
    output_task.create_negated_fluents()
    #print(f"Peak Memory before adding empty actions: {process.memory_info().rss / 1e6} MB")  # in bytes
    # Initialize actions
    print("Adding actions...")
    for (name, _) in nd_actions:
        output_task.add_action(name.encode('utf-8'))
    #print(f"Peak memory after adding empty actions: {process.memory_info().rss / 1e6} MB")  # in bytes
    ...
    # To access specific objects, we usually want to have Domain constants instead. So we deleted this.
